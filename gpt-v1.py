...
for iter in range(max_iters):
    ...
    if iter % eval_iters == 0:
        ...
        writer.add_scalar('Learning_Rate', learning_rate, iter)
        
        # Save model weights at every evaluation step
        checkpoint_path = f'model_checkpoint_step_{iter}.pth'
        with open(checkpoint_path, 'wb') as f:
            torch.save(model.state_dict(), checkpoint_path)
        print(f"Model saved at step {iter}: {checkpoint_path}")
    ...
    writer.add_scalar('Loss/Train_Step', loss.item(), iter)

# Save model weights atlast step
checkpoint_path = f'last.pth'
with open(checkpoint_path, 'wb') as f:
    torch.save(model.state_dict(), checkpoint_path)
print(f"Model saved at last step: {checkpoint_path}")
